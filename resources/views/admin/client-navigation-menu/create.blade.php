                            ...
                            @switch($item['type'])
                                @case('description')
                                    <div class="col-12">
                                        ...
                                        <textarea name="{{ $item['name'] }}" id="{{ $item['name'] }}" class="description" cols="30" rows="10" placeholder="{{ $item['placeholder'] }}"></textarea>
                                    </div>
                                    @break
                                @case('short_description')
                                    <div class="col-12">
                                        <label for="{{ $item['name'] }}" class="form-label"><b>{{ $item['label'] }}</b></label>
                                        <textarea name="{{ $item['name'] }}" id="{{ $item['name'] }}" class="short_description" cols="30" rows="10" placeholder="{{ $item['placeholder'] }}"></textarea>
                                    </div>
                                    @break
                                @case('select')
                                    <div class="col-12">
                                ...
                                @default
                                    <div class="col-12">
                                        ...
                                        <input type="{{ $item['type'] }}" value="{{ $item['value'] }}" class="form-control custom-input input-number" id="{{ $item['name'] }}" name="{{ $item['name'] }}" placeholder="{{ $item['placeholder'] }}">
                                    </div>
                            @endswitch
                        @endforeach
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        if(response.length){
                            ...
                            const option = $('<option></option>').attr('value', "").text('No Child Menu Found!');
                            $('#child_id').append(option); 
                        }
                    },
